...
def register_form_callbacks(dash_app):
    ...
    @dash_app.callback(
        ...
        Output('meme-id-store', 'value', allow_duplicate=True),
        Output('meme-name', 'value', allow_duplicate=True),
        Output('meme-description', 'value', allow_duplicate=True),
        Output('meme-ethical-dimension', 'value', allow_duplicate=True),
        Output('meme-tags', 'value', allow_duplicate=True),
        Output('morphisms-container', 'children', allow_duplicate=True),
        Output('mappings-container', 'children', allow_duplicate=True),
        Output('meme-is-merged', 'value', allow_duplicate=True),
        Output('meme-merged-from', 'value', allow_duplicate=True),
    ...
    def handle_save_meme(n_clicks, meme_id, name, description, dimension_values,
                         dynamic_input_values, dynamic_input_ids, tags,
                         morphism_types, morphism_targets, morphism_descs,
                         mapping_concepts, mapping_categories, mapping_types,
                         is_merged_value, merged_from_ids):
        """Handles saving/updating meme with dynamic relationships and merged status."""
        triggered_id = ctx.triggered_id
        if not triggered_id == 'save-meme-button':
            return no_update, False, *([no_update]*10)
        ...
        if errors:
            ...
            return error_message, True, *([no_update]*10) # Show errors, keep form state
        ...
        except Exception as e: logger.error(f"Error serializing payload: {e}", exc_info=True); return f"Internal error.", True, *([no_update]*10)
        try:
            ...
            else: error_data = response.json(); details = error_data.get('detail', f'Status {response.status_code}'); return f"Save failed: {details}", True, *([no_update]*10)
        except requests.exceptions.RequestException as e: return f"Network error: {e}", True, *([no_update]*10)
        except Exception as e: return f"Unexpected error: {e}", True, *([no_update]*10)
    ...
    @dash_app.callback(
        ...
        Output('dynamic-meme-attribute-inputs', 'children', allow_duplicate=True),
        Output('morphisms-container', 'children', allow_duplicate=True),
        Output('mappings-container', 'children', allow_duplicate=True),
    ...
    def handle_edit_selection(active_cell, table_data):
        """Populates static form fields and stores full data when a row is clicked."""
        # ... (Implementation from meme_management.py) ...
        if not active_cell or not table_data: return no_update, *([no_update]*7), None, [], [], []
        meme_id = table_data[active_cell['row']].get('_id'); logger.info(f"Storing data for editing meme ID: {meme_id}")
        full_meme_data = {}
        if meme_id:
            try: url = f"{BACKEND_API_URL}/{meme_id}"; response = requests.get(url, timeout=5); response.raise_for_status(); full_meme_data = loads(response.text)
            except Exception as e: logger.error(f"Error fetching/decoding full meme {meme_id}: {e}"); return no_update, *([no_update]*7), None, [], [], []
        else: return no_update, *([no_update]*7), None, [], [], []
        name = full_meme_data.get('name', ''); description = full_meme_data.get('description', '')
        dimensions = full_meme_data.get('ethical_dimension', []); tags = ", ".join(full_meme_data.get('tags', []) or [])
    ...
    # Callback to clear the form fields and stores
    @dash_app.callback(
        Output('meme-id-store', 'value', allow_duplicate=True),
        Output('meme-name', 'value', allow_duplicate=True),
        Output('meme-description', 'value', allow_duplicate=True),
        ...
        Output('meme-is-merged', 'value', allow_duplicate=True),
        Output('meme-merged-from', 'value', allow_duplicate=True),
        Output('edit-meme-store', 'data', allow_duplicate=True),
        Output('dynamic-meme-attribute-inputs', 'children', allow_duplicate=True),
        Input('clear-form-button', 'n_clicks'),
        prevent_initial_call=True
    )
    def clear_form(n_clicks):
        """Clears all form inputs and the edit store."""
        # ... (Implementation from meme_management.py) ...
